...
static Side side(AABB_Object a, AABB_Object b) {
...
void AABB_simulate(AABB_Object obj[], size_t objSize, float dt) {
  ...
  for (size_t i = 0; i < objSize; i++) {
    ...
    if (obj[i].x + obj[i].width > WIDTH) {
      obj[i].dx = -obj[i].dx;
      obj[i].x = WIDTH - obj[i].width;
    }
    if (obj[i].y < 0) {
      obj[i].dy = -obj[i].dy;
      obj[i].y = 0;
